...
function isEditing(){return document.body.dataset.wysiwygActive==='1'||document.body.isContentEditable;}
document.body.addEventListener('mousedown',function(e){if(isEditing())return;if(isInjected(e.target))return;clearCellSel();var cell=findCell(e.target);if(!cell)return;cellSelStart=cell;cellSelEnd=cell;mouseDownPos={x:e.clientX,y:e.clientY};cellSelMoved=false;},true);
document.body.addEventListener('mouseup',function(){if(isEditing()){clearCellSel();mouseDownPos=null;return;}if(!cellSelStart||!mouseDownPos)return;mouseDownPos=null;if(cellSelMoved){suppressNextClick=true;var tab=findTable(cellSelStart);var sp=cellPos(cellSelStart,tab);var ep=cellPos(cellSelEnd,tab);if(sp&&ep&&(sp[0]!==ep[0]||sp[1]!==ep[1]))showMergeBtn();else clearCellSel();}else{clearCellSel();}},true);
document.body.addEventListener('mousemove',function(e){if(isEditing())return;if(dragSrc)return;var t=e.target;if(mouseDownPos&&cellSelStart){var dx=e.clientX-mouseDownPos.x;var dy=e.clientY-mouseDownPos.y;if(dx*dx+dy*dy>=16){cellSelMoved=true;var c2=findCell(t);if(c2&&findTable(c2)===findTable(cellSelStart)&&c2!==cellSelEnd){cellSelEnd=c2;updateCellSel();}return;}}if(t===dragHandle||isInjected(t))return;if(t===document.body||t===document.documentElement){highlight(null);}else{highlight(t);}var d=findDraggable(t);if(d){showHandleOn(d);return;}if(hoverDrag){var rc=hoverDrag.getBoundingClientRect();var pad=80;if(e.clientX<rc.left-pad||e.clientX>rc.right+pad||e.clientY<rc.top-pad||e.clientY>rc.bottom+pad)showHandleOn(null);}},true);
document.body.addEventListener('mouseleave',function(){highlight(null);},true);
document.body.addEventListener('click',function(e){var t=e.target;if(suppressNextClick&&!isInjected(t)){suppressNextClick=false;e.preventDefault();e.stopPropagation();return;}if(isEditing())return;if(t===document.body||t===document.documentElement)return;if(isInjected(t))return;var path=pathOf(t);if(!path||path.length===0)return;e.preventDefault();e.stopPropagation();highlight(null);var isImg=t.tagName==='IMG';var isVideo=t.tagName==='VIDEO';var table=findTable(t);var tablePath=table?pathOf(table):null;var cell=findCell(t);var cp=cellPos(cell,table);parent.postMessage({type:'editor-element-click',path:path,tagPath:tagPathOf(t),isImg:isImg,isVideo:isVideo,imgIndex:isImg?imgIndex(t):-1,imgSrc:isImg?(t.getAttribute('src')||''):'',inTable:!!table,tablePath:tablePath,cellRowIdx:cp[0],cellColIdx:cp[1]},'*');},true);
...
function enter(){document.body.contentEditable='true';document.body.dataset.wysiwygActive='1';var imgs=document.querySelectorAll('img');for(var i=0;i<imgs.length;i++)imgs[i].contentEditable='false';var injected=document.querySelectorAll('[data-injected="1"]');for(var j=0;j<injected.length;j++){var el=injected[j];if(el.tagName==='DIV'||el.tagName==='BUTTON')el.style.display='none';}var cells=document.querySelectorAll('td,th');for(var k=0;k<cells.length;k++){cells[k].style.background='';cells[k].style.outline='';cells[k].style.outlineOffset='';}setTimeout(function(){document.body.focus();},50);}
